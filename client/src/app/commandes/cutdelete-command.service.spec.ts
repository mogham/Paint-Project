...
import { CutDeleteCommand } from './cutdelete-command.service';
describe('CutDeteleCommandService', () => {
  let fixture: ComponentFixture<DrawingViewComponent>;
  let drawingView: DrawingViewComponent;
  let renderer: Renderer2;
  let service: CutDeleteCommand;
  beforeEach(async(() => {
  ...
  it('should redoSVG', () => {
    ...
    service = new CutDeleteCommand([], selection);
    service['shapesToDelete'] = elementErased;
    ...
    service.redoSVG(shapes, svg);
    expect(service['shapesToRestore'].length).toEqual(2);
  ...
  it('should undoSVG', () => {
    ...
    rectHTMLElement.setAttribute('id', 'rect');
    rectHTMLElement.setAttribute('elementID', '1');
    elementToDelete.push(ellipseHTMLElement);
    elementToDelete.push(rectHTMLElement);
    const selection = new SelectionParamsService({} as ColorToolService,
                                                {} as IntersectionParamsService, {} as PaperweightManipulationService,
                                                {} as RotationManipulationService);
    service = new CutDeleteCommand(elementToDelete, selection);
    const shapes: Element[] = [];
    const svg = drawingView['svg'];
    service.undoSVG(shapes, svg, renderer);
    expect(service['shapesToDelete'].length).toEqual(2);
